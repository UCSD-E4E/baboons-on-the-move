...
def clip_frame(frame):
    padding = math.floor((frame.shape[1] - frame.shape[0]) / 2)

    return frame[:, padding:-padding]

fft_history_frames = deque([])
def push_history_frame(fft_frame):
    if len(fft_history_frames) == history_frame_count:
        fft_history_frames.popleft()

    fft_history_frames.append(fft_frame)
...
def main():  
    ...
    while(cap.isOpened()):
        ...
        if ret == True:
            ...
            fft_frame = np.fft.fft2(gray)
            
            cv2.imshow('Gray', cv2.resize(gray, (1600, 900)))

            # We need at least n frames to continue
            if (len(fft_history_frames) < history_frame_count):
                push_history_frame(fft_frame)
                continue

            shifted_history_frames = pool.map(shift_frame, [(fft_frame, f) for f in fft_history_frames])
            quantized_frames = pool.map(quantize, [f for f in shifted_history_frames])

            # shifted_history_frames = [shift_frame(fft_frame, f) for f in fft_history_frames]
            # quantized_frames = [quantize(f) for f in shifted_history_frames]
            ...
            if cv2.waitKey(25) & 0xFF == ord('q') or curr_time > 0.5 * 60 * 60:
                break
