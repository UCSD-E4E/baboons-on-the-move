...
class VariableBackgroundSub_ForegroundExtraction(ForegroundExtraction):
    ...
    def _quantize_frame(self, frame):
        ...
        """
        return (
            np.floor(frame.get_frame().astype(np.float32) * 50.0 / 255.0)
            .astype(np.uint8)
            .astype(np.int32)
        )
    ...
    def _get_moving_foreground(self, weights, foreground, dissimilarity):
        ...
        weights_medium = (
            np.logical_and(
                history_frame_count_third < weights, weights < self.history_frames - 1
            ).astype(np.uint8)
            * 2
        )
    ...
    def generate_mask(self, frame, shifted_history_frames, Ms):
        ...
        intersects = self._intersect_all_frames(
            grouped_shifted_history_frames, grouped_quantized_frames
        )
        union = self._union_frames(intersects)

        history_of_dissimilarity = self._get_history_of_dissimilarity(
        ...
        moving_foreground = self._get_moving_foreground(
            weights, foreground, history_of_dissimilarity
        )
