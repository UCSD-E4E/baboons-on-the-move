...
def draw_keypoints(vis, keypoints, color=(0, 255, 255)):
    for kp in keypoints:
        if kp.age > min_age:
            ...
            cv.line(
                vis, (int(x), int(y)), (int(x) + int(dx), int(y) + int(dy)), color, 2
            )
...
fgbg = cv.bgsegm.createBackgroundSubtractorMOG(
    bgsub_history, nmixtures, background_ratio
)
# fgbg = cv.createBackgroundSubtractorMOG2(500, 16, True)
out = cv.VideoWriter(
    "output.avi", fourcc, video_fps, (int(cap.get(3)), int(cap.get(4))), True
)
    ...
    def update(self):
        ...
        if self.updated == False:
            ...
            if length >= history:
                ...
                expected_position.pt = (
                    pt[0] + self.velocity[0],
                    pt[1] + self.velocity[1],
                )
                key.pt = (
                    key.pt[0] * (1 - momentum_ratio)
                    + expected_position.pt[0] * momentum_ratio,
                    key.pt[1] * (1 - momentum_ratio)
                    + expected_position.pt[1] * momentum_ratio,
                )
...
while 1:
    ...
    if ret == True:
        ...
        frame = cv.resize(
            framedis,
            (int(cap.get(3) / 2.3), int(cap.get(4) / 2.3)),
