...
config[
    "k_nearest_baboons"
] = 4  # getting the four nearest baboons per frame to consider(MUST BE EVEN)
config["k_historic_velocities"] = 4  # getting the four past velocities to consider
config["checkpoint_training"] = False
config[
    "saved_path"
] = "saved_weights.pth"  # path to save current model information (epoch, weights, etc...)
config["learning_rate"] = 0.0001  # lr
config[
    "batch_size"
] = 32  # Number of training samples per batch to be passed to network
config["val_split"] = 0.2  # ratio of val and training data
config[
    "shuffle_dataset"
] = True  # shuffle data before start (TODO: change this if we use kfold?)
config["epochs"] = 100  # Number of epochs to train the model
config["early_stop"] = True  # Implement early stopping or not
config[
    "early_stop_epoch"
] = 3  # Number of epochs for which validation loss increases to be counted as overfitting
...
config["input_dimension"] = (
    config["k_nearest_baboons"] + config["k_historic_velocities"] + 1
)  # DON'T TOUCH THIS
...
frames_to_velocities = (
    {}
)  # Maps a single frame to a list of all the baboons, their id, position, and velocity
...
for idx, row in training_data.iterrows():
    ...
    frames_to_velocities[current_frame].append(
        (current_position, current_velocity, current_id)
    )
...
def get_k_nearest_baboons_velocities(frame, baboon_id):
    ...
    if len(baboons_in_frame) <= config["k_nearest_baboons"]:
        padded_output = list(
            np.ones(config["k_nearest_baboons"] - len(baboons_in_frame)) * -1
        )
    ...
    for k_nearest_neighbor in range(config["k_nearest_baboons"]):
        ...
        for idx, baboon in enumerate(baboons_in_frame):
            current_distance = np.linalg.norm(
                np.array(baboon[0]) - np.array(target_velocity)
            )
...
baboons_to_velocities = (
    {}
)  # maps the baboon's id to it's velocties and respective frames in a tuple
...
def get_k_past_velocities(current_frame, baboon_id):
    ...
    if frame_index < config["k_historic_velocities"]:
        padded_output = list(
            np.ones(config["k_historic_velocities"] - frame_index) * -1
        )
        ...
        return [
            ...
            for item in velocities[
                frame_index - config["k_historic_velocities"] : frame_index
            ]
        ]
...
for idx, row in training_data.iterrows():
    ...
    current_label = training_data.loc[
        (training_data["frame"] == (current_frame + 1))
        & (training_data["baboon id"] == current_id)
    ]
    ...
    current_datapoint[1:start_historic] = get_k_nearest_baboons_velocities(
        current_frame, current_id
    )
    current_datapoint[start_historic:] = get_k_past_velocities(
        current_frame, current_id
    )
