...
if not config["checkpoint_training"]:
    net = Nnet(config["input_dimension"], config["output_dimension"]).to(
        computing_device
    )
...
for epoch in range(config["epochs"]):
    ...
    for minibatch_count, (datapoints, labels) in enumerate(train_loader, 0):
        ...
        datapoints, labels = (
            datapoints.to(computing_device),
        ...
        if minibatch_count % N == 49:
            ...
            N_minibatch_loss /= N
            print(
                f"Epoch {epoch + 1}, average minibatch {minibatch_count+1} loss: {N_minibatch_loss}"
            )
    ...
    print(
        f"Finished {epoch+ 1} epochs of training with average {average_epoch_loss} loss."
    )
    ...
    with torch.no_grad():
        ...
        print(
            f"Epoch {epoch + 1} average validation loss over {val_data_size} datapoints : {N_minibatch_val_loss}"
        )
